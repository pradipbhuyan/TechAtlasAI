"""
CloudPulse AI - Executive Cloud Intelligence Briefing
...
DOMAIN_SECTIONS = [
    ...
    "edge_sovereign_hybrid_cloud",
    "ai_cloud_services",
    "strategic_deals_partnerships",
]
DOMAIN_LABELS = {
    "hyperscaler_updates": ("☁️ AWS, Azure & Google Cloud Updates", "tag-strategy"),
    "finops_cost_optimization": ("💰 FinOps & Cloud Cost Signals", "tag-strategy"),
    "kubernetes_cloud_native": ("⚙️ Kubernetes & Cloud-Native Platforms", "tag-strategy"),
    "platform_engineering_idp": ("🧰 Platform Engineering & IDPs", "tag-strategy"),
    "cloud_security_compliance": ("🛡️ Cloud Security & Compliance", "tag-strategy"),
    "edge_sovereign_hybrid_cloud": ("🌐 Edge, Sovereign & Hybrid Cloud", "tag-strategy"),
    "ai_cloud_services": ("🤖 AI Cloud Services", "tag-strategy"),
...
# Prompts
# -----------------------------------------------------------------------------
DRAFT_PROMPT = """You are CloudPulse, a senior analyst and broadcast journalist covering cloud platforms, hyperscalers, FinOps, cloud-native operations and platform engineering.
Today is __DATE__.
Briefing type: __BRIEFING_TYPE__.

Generate a premium cloud intelligence briefing for executives. Prioritize fresh variety, strategic relevance and delivery impact.

Core coverage areas: hyperscaler updates, AWS, Microsoft Azure, Google Cloud, cloud-native platforms, Kubernetes, containers, FinOps, cost optimization, platform engineering, internal developer platforms, cloud security, compliance, sovereign cloud, edge, hybrid cloud, AI cloud services, deals and partnerships.

QUALITY STANDARDS:
- Use real industry context, standards bodies, vendors, platforms, enterprise adoption patterns and market dynamics.
- Reference relevant entities such as AWS, Microsoft Azure, Google Cloud, Oracle Cloud, IBM Cloud, VMware/Broadcom, Red Hat, HashiCorp, Datadog, Dynatrace, ServiceNow, Snowflake, Databricks, Kubernetes, CNCF, FinOps Foundation, Cloud Native Computing Foundation, major global enterprises and systems integrators where relevant.
...
"briefing_type", "executive_summary", "top_developments", "market_signals_vendor_moves", "hyperscaler_updates", "finops_cost_optimization", "kubernetes_cloud_native", "platform_engineering_idp", "cloud_security_compliance", "edge_sovereign_hybrid_cloud", "ai_cloud_services", "strategic_deals_partnerships", "key_deals", "analyst_take", "what_to_watch_next", "tts_script".
...
REVIEW_PROMPT = """You are the Chief Editorial Officer of CloudPulse.
...
TTS_PROMPT = """You are CloudPulse, a professional broadcast scriptwriter for senior executives.
...
def email_subject(report: dict) -> str:
    return f"{APP_NAME} {textify(report.get('briefing_type', 'Executive Cloud Intelligence Briefing'))} | {today_str()}"
...
def build_email_summary(report: dict) -> dict:
    ...
    return {
        "intro": "Please find attached the latest CloudPulse AI Intelligence Briefing, prepared for cloud stakeholders, architects and delivery leaders.",
...
<tr><td style='padding:30px 34px;background:#0f172a;color:white;'><div style='font-family:Arial;font-size:28px;font-weight:800;'>{APP_ICON} {APP_NAME}</div><div style='font-family:Arial;font-size:11px;letter-spacing:2px;color:#9bdcf5;text-transform:uppercase;'>Executive Cloud Intelligence Briefing</div><div style='font-family:Arial;color:#cbd5e1;font-size:13px;margin-top:16px;'>{escape(textify(report.get('briefing_type','Executive Cloud Intelligence Briefing')))} - {escape(datetime.now().strftime('%A, %B %d, %Y'))}</div></td></tr>
...
def generate_pdf_report(report: dict, output_path: str):
    ...
    story += [logo_table, Spacer(1, 8), Paragraph(APP_NAME, title_style), Paragraph("Strategic Telecom Intelligence", logo_style), Paragraph(f"{escape(textify(report.get('briefing_type', 'Executive Cloud Intelligence Briefing')))} - {datetime.now().strftime('%A, %B %d, %Y')}", subtitle_style)]
    ...
    if top_devs:
        story.append(Paragraph("Top 5 Cloud Developments", h_style))
...
with st.sidebar:
    ...
    st.caption("Executive Cloud Intelligence Briefing")
    st.markdown("---")
    st.markdown("### ⚙️ Configuration")
    st.markdown("**🔑 OpenAI**")
    st.success("OpenAI key loaded" if OPENAI_API_KEY else "Set OPENAI_API_KEY in secrets/env")
    briefing_type = st.selectbox("Briefing Type", ["Weekly Cloud Intelligence Brief", "Daily Cloud Flash Brief"], index=0)
    ...
    with st.status("📝 Stage 1 · Generating draft briefing...", expanded=True) as s1:
        try:
            ...
            if use_cache and draft_cache.exists():
                ...
                st.write(f"🧠 Generating {briefing_type.lower()} across expanded cloud topics...")
...
if final:
    ...
    with t_report:
        ...
        if top_devs:
            st.markdown("### 🧭 Top Cloud Developments")
        ...
        txt_parts = [
            f"CLOUDPULSE {textify(final.get('briefing_type', 'BRIEFING')).upper()} - {today_str()}",
            "=" * 72,
            CREATOR_FOOTNOTE,
            "EXECUTIVE SUMMARY",
