"""
EnterpriseApps Pulse - Executive Enterprise Applications Intelligence Briefing
...
APP_NAME = "EnterpriseApps Pulse"
APP_ICON = "🏢"
APP_TAGLINE = "Executive Enterprise Applications Intelligence Briefing"
FILE_PREFIX = "enterprise_apps_pulse"
APP_KEY = FILE_PREFIX
...
DOMAIN_SECTIONS = [
    "erp_finance_operations",
    "crm_cx_commerce",
    "workflow_itsm_service_ops",
    "hcm_workforce_productivity",
    "low_code_automation",
    "embedded_ai_copilots",
    "modernization_integration_data",
    "vendor_deals_partnerships",
]
DOMAIN_LABELS = {
    "erp_finance_operations": ("🏛️ ERP, Finance & Operations", "tag-strategy"),
    "crm_cx_commerce": ("💼 CRM, CX & Commerce", "tag-strategy"),
    "workflow_itsm_service_ops": ("🧩 Workflow, ITSM & Service Ops", "tag-strategy"),
    "hcm_workforce_productivity": ("👥 HCM & Workforce Productivity", "tag-strategy"),
    "low_code_automation": ("🛠️ Low-Code & Automation", "tag-strategy"),
    "embedded_ai_copilots": ("🤖 Embedded AI & Copilots", "tag-strategy"),
    "modernization_integration_data": ("🔗 Modernization, Integration & Data", "tag-strategy"),
    "vendor_deals_partnerships": ("🤝 Vendor Deals & Partnerships", "tag-strategy"),
}
...
# Prompts
# -----------------------------------------------------------------------------
DRAFT_PROMPT = """You are EnterpriseApps Pulse, a senior analyst and broadcast journalist covering enterprise applications, ERP, CRM, workflow, SaaS modernization and embedded AI.
Today is __DATE__.
Briefing type: __BRIEFING_TYPE__.

Generate a premium enterprise applications intelligence briefing for executives. Prioritize fresh variety, strategic relevance and delivery impact.

Core coverage areas: ERP, finance, supply chain, CRM, CX, commerce, ITSM, workflow automation, HCM, low-code platforms, embedded AI, copilots, enterprise SaaS modernization, integration, data, vendor deals and partnerships.

QUALITY STANDARDS:
- Use real industry context, standards bodies, vendors, platforms, enterprise adoption patterns and market dynamics.
- Reference relevant entities such as SAP, Salesforce, ServiceNow, Oracle, Workday, Microsoft Dynamics, Adobe, Atlassian, UiPath, Automation Anywhere, Celonis, Boomi, MuleSoft, IBM, AWS, Microsoft and Google Cloud enterprise application ecosystems where relevant.
- Do not invent specific deal values, breach details, benchmark results, contract wins or regulatory deadlines. If exact value is not known, use "Not disclosed".
- Distinguish announced capabilities from production-ready enterprise impact.
- Write at Gartner/Forrester/IDC analyst-advisory level.

Return a JSON object with exactly these keys:
"briefing_type", "executive_summary", "top_developments", "market_signals_vendor_moves", "erp_finance_operations", "crm_cx_commerce", "workflow_itsm_service_ops", "hcm_workforce_productivity", "low_code_automation", "embedded_ai_copilots", "modernization_integration_data", "vendor_deals_partnerships", "key_deals", "analyst_take", "what_to_watch_next", "tts_script".
...
REVIEW_PROMPT = """You are the Chief Editorial Officer of EnterpriseApps Pulse.
Review the draft across content clarity, authenticity, strategic relevance and professionalism.
...
TTS_PROMPT = """You are EnterpriseApps Pulse, a professional broadcast scriptwriter for senior executives.
...
def email_subject(report: dict) -> str:
    return f"{APP_NAME} {textify(report.get('briefing_type', 'Executive Enterprise Applications Intelligence Briefing'))} | {today_str()}"
...
def build_email_summary(report: dict) -> dict:
    ...
    return {
        "intro": "Please find attached the latest EnterpriseApps Pulse Intelligence Briefing, prepared for enterprise application leaders, account teams, architects and delivery leaders.",
...
<tr><td style='padding:30px 34px;background:#0f172a;color:white;'><div style='font-family:Arial;font-size:28px;font-weight:800;'>{APP_ICON} {APP_NAME}</div><div style='font-family:Arial;font-size:11px;letter-spacing:2px;color:#9bdcf5;text-transform:uppercase;'>Executive Enterprise Applications Intelligence Briefing</div><div style='font-family:Arial;color:#cbd5e1;font-size:13px;margin-top:16px;'>{escape(textify(report.get('briefing_type','Executive Enterprise Applications Intelligence Briefing')))} - {escape(datetime.now().strftime('%A, %B %d, %Y'))}</div></td></tr>
...
def generate_pdf_report(report: dict, output_path: str):
    ...
    story += [logo_table, Spacer(1, 8), Paragraph(APP_NAME, title_style), Paragraph("Strategic Telecom Intelligence", logo_style), Paragraph(f"{escape(textify(report.get('briefing_type', 'Executive Enterprise Applications Intelligence Briefing')))} - {datetime.now().strftime('%A, %B %d, %Y')}", subtitle_style)]
    ...
    if top_devs:
        story.append(Paragraph("Top 5 Enterprise Applications Developments", h_style))
...
with st.sidebar:
    st.markdown("### 📡 EnterpriseApps Pulse")
    st.caption("Executive Enterprise Applications Intelligence Briefing")
    st.markdown("---")
    st.markdown("### ⚙️ Configuration")
    st.markdown("**🔑 OpenAI**")
    st.success("OpenAI key loaded" if OPENAI_API_KEY else "Set OPENAI_API_KEY in secrets/env")
    briefing_type = st.selectbox("Briefing Type", ["Weekly Enterprise Apps Intelligence Brief", "Daily Enterprise Apps Flash Brief"], index=0)
    ...
    with st.status("📝 Stage 1 · Generating draft briefing...", expanded=True) as s1:
        try:
            ...
            if use_cache and draft_cache.exists():
                ...
                st.write(f"🧠 Generating {briefing_type.lower()} across expanded enterprise applications topics...")
...
if final:
    ...
    with t_report:
        ...
        if top_devs:
            st.markdown("### 🧭 Top Enterprise Applications Developments")
        ...
        txt_parts = [
            f"ENTERPRISEAPPSPULSE {textify(final.get('briefing_type', 'BRIEFING')).upper()} - {today_str()}",
            "=" * 72,
            CREATOR_FOOTNOTE,
            "EXECUTIVE SUMMARY",
