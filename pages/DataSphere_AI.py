"""
DataSphere AI - Executive Data & Analytics Intelligence Briefing
...
APP_NAME = "DataSphere AI"
APP_ICON = "🗃️"
APP_TAGLINE = "Executive Data & Analytics Intelligence Briefing"
FILE_PREFIX = "data_sphere"
APP_KEY = FILE_PREFIX
...
DOMAIN_SECTIONS = [
    "lakehouse_warehouse_platforms",
    "data_engineering_pipelines",
    "analytics_bi_decisioning",
    "data_governance_catalog_quality",
    "real_time_streaming",
    "mlops_llmops_ai_data",
    "privacy_security_compliance",
    "vendor_deals_ecosystem",
]
DOMAIN_LABELS = {
    "lakehouse_warehouse_platforms": ("🏛️ Lakehouse & Warehouse Platforms", "tag-strategy"),
    "data_engineering_pipelines": ("🔁 Data Engineering & Pipelines", "tag-strategy"),
    "analytics_bi_decisioning": ("📊 Analytics, BI & Decisioning", "tag-strategy"),
    "data_governance_catalog_quality": ("🧭 Governance, Catalog & Quality", "tag-strategy"),
    "real_time_streaming": ("⚡ Real-Time & Streaming Data", "tag-strategy"),
    "mlops_llmops_ai_data": ("🤖 MLOps, LLMOps & AI Data", "tag-strategy"),
    "privacy_security_compliance": ("🔐 Privacy, Security & Compliance", "tag-strategy"),
    "vendor_deals_ecosystem": ("🤝 Vendor Deals & Ecosystem", "tag-strategy"),
}
...
# Prompts
# -----------------------------------------------------------------------------
DRAFT_PROMPT = """You are DataSphere, a senior analyst and broadcast journalist covering data platforms, analytics, lakehouse, governance, BI and AI-ready data architecture.
Today is __DATE__.
Briefing type: __BRIEFING_TYPE__.

Generate a premium data & analytics intelligence briefing for executives. Prioritize fresh variety, strategic relevance and delivery impact.

Core coverage areas: data platforms, lakehouse, data warehouse, analytics, BI, data engineering, pipelines, data governance, catalogs, data quality, observability, real-time streaming, MLOps, LLMOps, privacy, compliance, vendor deals and ecosystem partnerships.

QUALITY STANDARDS:
- Use real industry context, standards bodies, vendors, platforms, enterprise adoption patterns and market dynamics.
- Reference relevant entities such as Databricks, Snowflake, Microsoft Fabric, Google BigQuery, AWS Redshift, Oracle, IBM, Informatica, Collibra, Alation, dbt Labs, Confluent, MongoDB, Elastic, Tableau, Power BI and major enterprise data platform providers where relevant.
...
"briefing_type", "executive_summary", "top_developments", "market_signals_vendor_moves", "lakehouse_warehouse_platforms", "data_engineering_pipelines", "analytics_bi_decisioning", "data_governance_catalog_quality", "real_time_streaming", "mlops_llmops_ai_data", "privacy_security_compliance", "vendor_deals_ecosystem", "key_deals", "analyst_take", "what_to_watch_next", "tts_script".
...
REVIEW_PROMPT = """You are the Chief Editorial Officer of DataSphere.
...
TTS_PROMPT = """You are DataSphere, a professional broadcast scriptwriter for senior executives.
...
def email_subject(report: dict) -> str:
    return f"{APP_NAME} {textify(report.get('briefing_type', 'Executive Data & Analytics Intelligence Briefing'))} | {today_str()}"
...
def build_email_summary(report: dict) -> dict:
    ...
    return {
        "intro": "Please find attached the latest DataSphere AI Intelligence Briefing, prepared for data leaders, analytics teams, architects and delivery leaders.",
...
<tr><td style='padding:30px 34px;background:#0f172a;color:white;'><div style='font-family:Arial;font-size:28px;font-weight:800;'>{APP_ICON} {APP_NAME}</div><div style='font-family:Arial;font-size:11px;letter-spacing:2px;color:#9bdcf5;text-transform:uppercase;'>Executive Data & Analytics Intelligence Briefing</div><div style='font-family:Arial;color:#cbd5e1;font-size:13px;margin-top:16px;'>{escape(textify(report.get('briefing_type','Executive Data & Analytics Intelligence Briefing')))} - {escape(datetime.now().strftime('%A, %B %d, %Y'))}</div></td></tr>
...
def generate_pdf_report(report: dict, output_path: str):
    ...
    story += [logo_table, Spacer(1, 8), Paragraph(APP_NAME, title_style), Paragraph("Strategic Telecom Intelligence", logo_style), Paragraph(f"{escape(textify(report.get('briefing_type', 'Executive Data & Analytics Intelligence Briefing')))} - {datetime.now().strftime('%A, %B %d, %Y')}", subtitle_style)]
    ...
    if top_devs:
        story.append(Paragraph("Top 5 Data & Analytics Developments", h_style))
...
with st.sidebar:
    st.markdown("### 📡 DataSphere AI")
    st.caption("Executive Data & Analytics Intelligence Briefing")
    st.markdown("---")
    st.markdown("### ⚙️ Configuration")
    st.markdown("**🔑 OpenAI**")
    st.success("OpenAI key loaded" if OPENAI_API_KEY else "Set OPENAI_API_KEY in secrets/env")
    briefing_type = st.selectbox("Briefing Type", ["Weekly Data Intelligence Brief", "Daily Data Flash Brief"], index=0)
    ...
    with st.status("📝 Stage 1 · Generating draft briefing...", expanded=True) as s1:
        try:
            ...
            if use_cache and draft_cache.exists():
                ...
                st.write(f"🧠 Generating {briefing_type.lower()} across expanded data & analytics topics...")
...
if final:
    ...
    with t_report:
        ...
        if top_devs:
            st.markdown("### 🧭 Top Data & Analytics Developments")
        ...
        txt_parts = [
            f"DATASPHERE {textify(final.get('briefing_type', 'BRIEFING')).upper()} - {today_str()}",
            "=" * 72,
            CREATOR_FOOTNOTE,
            "EXECUTIVE SUMMARY",
