"""
NeuralPulse AI - AI Intelligence Digest
Daily and weekly executive digest for the field of Artificial Intelligence.

Features:
- Streamlit secrets or environment variable credential loading
- Daily / weekly AI digest generation with OpenAI
- Internal editorial review
- PDF report generation
- MP3 audio briefing generation with edge-tts
- Automatic and manual email delivery
- Gmail-safe attachment-size guard
- Downloadable branded email HTML/TXT assets
...
APP_NAME = "NeuralBrief AI"
APP_ICON = "🧠"
APP_TAGLINE = "Executive AI Intelligence Digest"
FILE_PREFIX = "neuralpulse_ai"
...
DOMAIN_SECTIONS = [
    "foundation_models_agents",
    "enterprise_ai_adoption",
    "ai_infrastructure_chips_cloud",
    "data_platforms_mlops_llmops",
    "ai_security_safety_governance",
    "regulation_policy_standards",
    "vertical_industry_use_cases",
    "startup_funding_mna_research",
]
DOMAIN_LABELS = {
    "foundation_models_agents": ("🧠 Foundation Models & Agents", "tag-ai"),
    "enterprise_ai_adoption": ("🏢 Enterprise AI Adoption", "tag-enterprise"),
    "ai_infrastructure_chips_cloud": ("⚡ AI Infrastructure, Chips & Cloud", "tag-infra"),
    "data_platforms_mlops_llmops": ("🗃️ Data Platforms, MLOps & LLMOps", "tag-data"),
    "ai_security_safety_governance": ("🛡️ AI Security, Safety & Governance", "tag-risk"),
    "regulation_policy_standards": ("⚖️ Regulation, Policy & Standards", "tag-policy"),
    "vertical_industry_use_cases": ("🏭 Industry Use Cases", "tag-industry"),
    "startup_funding_mna_research": ("🚀 Startups, Funding, M&A & Research", "tag-market"),
}
...
# Prompts
# -----------------------------------------------------------------------------
DRAFT_PROMPT = """You are NeuralPulse AI, a senior AI analyst and broadcast journalist covering the global artificial intelligence ecosystem.
Today is __DATE__.
Briefing type: __BRIEFING_TYPE__.

Generate a premium AI intelligence digest for senior executives, technology leaders and delivery leaders. Prioritize strategic relevance, business impact and fresh variety.

Core coverage areas: foundation models, generative AI, agentic AI, enterprise AI adoption, AI infrastructure, GPUs/accelerators, cloud AI platforms, data platforms, MLOps/LLMOps, AI security, model safety, governance, regulation, standards, industry use cases, startup funding, M&A, research breakthroughs and ecosystem partnerships.

QUALITY STANDARDS:
- Use real AI industry context, major vendors, cloud providers, chipmakers, research labs, standards bodies and enterprise adoption dynamics.
- Reference relevant entities such as OpenAI, Anthropic, Google DeepMind, Microsoft, Meta, Amazon, NVIDIA, AMD, Intel, Apple, IBM, Oracle, Salesforce, ServiceNow, Databricks, Snowflake, Hugging Face, Mistral AI, Cohere, xAI, Stability AI, Scale AI, MLCommons, NIST, ISO/IEC, OECD, EU AI Office and major enterprise adopters where relevant.
- Do not invent specific deal values, product launches, benchmark results or regulatory dates. If exact value is not known, use "Not disclosed".
- Avoid hype. Distinguish announced capabilities from production-ready enterprise impact.
- Write at Gartner/Forrester/IDC/CB Insights analyst-advisory level.

Return a JSON object with exactly these keys:
"briefing_type", "executive_summary", "top_developments", "market_signals_vendor_moves", "foundation_models_agents", "enterprise_ai_adoption", "ai_infrastructure_chips_cloud", "data_platforms_mlops_llmops", "ai_security_safety_governance", "regulation_policy_standards", "vertical_industry_use_cases", "startup_funding_mna_research", "key_deals", "analyst_take", "what_to_watch_next", "tts_script".
...
REVIEW_PROMPT = """You are the Chief Editorial Officer of NeuralPulse AI.
Review the draft across clarity, authenticity, professional tone, strategic relevance and AI-market accuracy.
...
TTS_PROMPT = """You are NeuralPulse AI, a professional broadcast scriptwriter for senior executives following the AI market.
...
def briefing_length_guidance(briefing_type: str) -> str:
    if briefing_type.startswith("Daily"):
        return "Daily AI Flash Digest: 900-1,300 words total. Key deals/signals: 3-5."
    return "Weekly AI Intelligence Digest: 3,500-5,000 words total. Key deals/signals: 8-12."
...
def email_subject(report: dict) -> str:
    return f"{APP_NAME} {textify(report.get('briefing_type', 'AI Intelligence Digest'))} | {today_str()}"
...
def build_email_summary(report: dict) -> dict:
    ...
    return {
        "intro": "Please find attached the latest NeuralPulse AI Intelligence Digest, prepared for senior AI, technology and delivery leaders.",
...
<tr><td style='padding:30px 34px;background:#0f172a;color:white;'><div style='font-family:Arial;font-size:28px;font-weight:800;'>{APP_ICON} {APP_NAME}</div><div style='font-family:Arial;font-size:11px;letter-spacing:2px;color:#9bdcf5;text-transform:uppercase;'>Executive AI Intelligence Digest</div><div style='font-family:Arial;color:#cbd5e1;font-size:13px;margin-top:16px;'>{escape(textify(report.get('briefing_type','AI Intelligence Digest')))} - {escape(datetime.now().strftime('%A, %B %d, %Y'))}</div></td></tr>
...
def generate_pdf_report(report: dict, output_path: str):
    ...
    story += [logo_table, Spacer(1, 8), Paragraph(APP_NAME, title_style), Paragraph("Strategic AI Intelligence", logo_style), Paragraph(f"{escape(textify(report.get('briefing_type', 'AI Intelligence Digest')))} - {datetime.now().strftime('%A, %B %d, %Y')}", subtitle_style)]
    ...
    if deals:
        story += [PageBreak(), Paragraph("Key Deals, Partnerships & Market Signals", h_style)]
...
with st.sidebar:
    st.markdown("### 🧠 NeuralPulse AI")
    st.caption("Executive AI Intelligence Digest")
    st.markdown("---")
    st.markdown("### ⚙️ Configuration")
    st.markdown("**🔑 OpenAI**")
    st.success("OpenAI key loaded" if OPENAI_API_KEY else "Set OPENAI_API_KEY in secrets/env")
    briefing_type = st.selectbox("Briefing Type", ["Weekly AI Intelligence Digest", "Daily AI Flash Digest"], index=0)
...
if run_btn:
    ...
    with st.status("📝 Stage 1 · Generating draft AI digest...", expanded=True) as s1:
        try:
            ...
            if use_cache and draft_cache.exists():
                ...
                st.write(f"🧠 Generating {briefing_type.lower()} across expanded AI topics...")
        ...
        try:
            if force_mp3 or not os.path.exists(mp3_out):
                if use_cache and tts_cache.exists():
                    ...
                    st.write("🗣️ Generating dedicated spoken AI briefing script...")
...
if final:
    if review:
        ...
        st.markdown('<div class="approved-banner" style="background:linear-gradient(135deg,#1a1208,#2d1f00);border-color:#f59e0b;color:#fcd34d;">📋 Digest ready · Editorial review was skipped.</div>', unsafe_allow_html=True)
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    t_report, t_review, t_audio, t_email_copy = st.tabs(["📋 Final Digest", "🔍 Internal Editorial Review", "🎧 Audio, PDF & Delivery", "✉️ Email Copy"])
    ...
    with t_report:
        ...
        if top_devs:
            st.markdown("### 🧭 Top AI Developments")
        ...
        if deals:
            st.markdown("### 🔑 Key Deals, Partnerships & Market Signals")
            for deal in deals:
                if isinstance(deal, dict):
        ...
        txt_parts = [
            f"{APP_NAME.upper()} {textify(final.get('briefing_type', 'DIGEST')).upper()} - {today_str()}",
            "=" * 72,
            CREATOR_FOOTNOTE,
            "EXECUTIVE SUMMARY",
        ...
        st.download_button("⬇️ Download Final Digest (TXT)", data=txt, file_name=f"{FILE_PREFIX}_{active_report_id()}_final.txt", mime="text/plain")
    ...
    with t_audio:
        ...
        if st.session_state.mp3_ready and mp3_file.exists():
            ...
            st.info("🎙️ MP3 not yet generated. Click Generate Digest to create it.")
        st.markdown("---")
        ...
        if pdf_file.exists():
            ...
            st.info("📄 PDF report not yet generated. Click Generate Digest to create it.")
        st.markdown("---")
        st.markdown("### 📧 Email Delivery")
