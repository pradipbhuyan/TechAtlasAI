"""
CyberShield Brief - Executive Cybersecurity Intelligence Briefing
...
APP_NAME = "CyberShield Brief"
APP_ICON = "🛡️"
APP_TAGLINE = "Executive Cybersecurity Intelligence Briefing"
FILE_PREFIX = "cyber_shield"
APP_KEY = FILE_PREFIX
...
DOMAIN_SECTIONS = [
    "threat_landscape_incidents",
    "vulnerabilities_cves_patching",
    "ransomware_extortion",
    "identity_zero_trust",
    "cloud_app_security",
    "security_operations_ai",
    "regulation_compliance_privacy",
    "vendor_moves_deals",
]
DOMAIN_LABELS = {
    "threat_landscape_incidents": ("🚨 Threat Landscape & Incidents", "tag-strategy"),
    "vulnerabilities_cves_patching": ("🧩 Vulnerabilities, CVEs & Patching", "tag-strategy"),
    "ransomware_extortion": ("💣 Ransomware & Extortion", "tag-strategy"),
    "identity_zero_trust": ("🪪 Identity & Zero Trust", "tag-strategy"),
    "cloud_app_security": ("☁️ Cloud & Application Security", "tag-strategy"),
    "security_operations_ai": ("🤖 Security Operations & AI", "tag-strategy"),
    "regulation_compliance_privacy": ("⚖️ Regulation, Compliance & Privacy", "tag-strategy"),
    "vendor_moves_deals": ("🤝 Vendor Moves & Deals", "tag-strategy"),
}
...
# Prompts
# -----------------------------------------------------------------------------
DRAFT_PROMPT = """You are CyberShield, a senior analyst and broadcast journalist covering cybersecurity, threat intelligence, vulnerabilities, identity, cloud security and cyber regulation.
Today is __DATE__.
Briefing type: __BRIEFING_TYPE__.

Generate a premium cybersecurity intelligence briefing for executives. Prioritize fresh variety, strategic relevance and delivery impact.

Core coverage areas: threat landscape, major incidents, vulnerabilities and CVEs, patching priorities, ransomware, extortion, identity and zero trust, cloud security, application security, security operations, AI in cybersecurity, privacy, compliance, deals and vendor moves.

QUALITY STANDARDS:
- Use real industry context, standards bodies, vendors, platforms, enterprise adoption patterns and market dynamics.
- Reference relevant entities such as Microsoft Security, CrowdStrike, Palo Alto Networks, Fortinet, Cisco, Zscaler, Okta, Wiz, SentinelOne, Mandiant, Google Cloud Security, AWS Security, CISA, NIST, ENISA, MITRE ATT&CK and major security research teams where relevant.
...
"briefing_type", "executive_summary", "top_developments", "market_signals_vendor_moves", "threat_landscape_incidents", "vulnerabilities_cves_patching", "ransomware_extortion", "identity_zero_trust", "cloud_app_security", "security_operations_ai", "regulation_compliance_privacy", "vendor_moves_deals", "key_deals", "analyst_take", "what_to_watch_next", "tts_script".
...
REVIEW_PROMPT = """You are the Chief Editorial Officer of CyberShield.
...
TTS_PROMPT = """You are CyberShield, a professional broadcast scriptwriter for senior executives.
...
def email_subject(report: dict) -> str:
    return f"{APP_NAME} {textify(report.get('briefing_type', 'Executive Cybersecurity Intelligence Briefing'))} | {today_str()}"
...
def build_email_summary(report: dict) -> dict:
    ...
    return {
        "intro": "Please find attached the latest CyberShield Brief Intelligence Briefing, prepared for CISOs, technology executives, risk leaders and delivery leaders.",
...
<tr><td style='padding:30px 34px;background:#0f172a;color:white;'><div style='font-family:Arial;font-size:28px;font-weight:800;'>{APP_ICON} {APP_NAME}</div><div style='font-family:Arial;font-size:11px;letter-spacing:2px;color:#9bdcf5;text-transform:uppercase;'>Executive Cybersecurity Intelligence Briefing</div><div style='font-family:Arial;color:#cbd5e1;font-size:13px;margin-top:16px;'>{escape(textify(report.get('briefing_type','Executive Cybersecurity Intelligence Briefing')))} - {escape(datetime.now().strftime('%A, %B %d, %Y'))}</div></td></tr>
...
def generate_pdf_report(report: dict, output_path: str):
    ...
    story += [logo_table, Spacer(1, 8), Paragraph(APP_NAME, title_style), Paragraph("Strategic Telecom Intelligence", logo_style), Paragraph(f"{escape(textify(report.get('briefing_type', 'Executive Cybersecurity Intelligence Briefing')))} - {datetime.now().strftime('%A, %B %d, %Y')}", subtitle_style)]
    ...
    if top_devs:
        story.append(Paragraph("Top 5 Cybersecurity Developments", h_style))
...
with st.sidebar:
    st.markdown("### 📡 CyberShield Brief")
    st.caption("Executive Cybersecurity Intelligence Briefing")
    st.markdown("---")
    st.markdown("### ⚙️ Configuration")
    st.markdown("**🔑 OpenAI**")
    st.success("OpenAI key loaded" if OPENAI_API_KEY else "Set OPENAI_API_KEY in secrets/env")
    briefing_type = st.selectbox("Briefing Type", ["Weekly Cybersecurity Intelligence Brief", "Daily Cyber Flash Brief"], index=0)
    ...
    with st.status("📝 Stage 1 · Generating draft briefing...", expanded=True) as s1:
        try:
            ...
            if use_cache and draft_cache.exists():
                ...
                st.write(f"🧠 Generating {briefing_type.lower()} across expanded cybersecurity topics...")
...
if final:
    ...
    with t_report:
        ...
        if top_devs:
            st.markdown("### 🧭 Top Cybersecurity Developments")
        ...
        txt_parts = [
            f"CYBERSHIELD {textify(final.get('briefing_type', 'BRIEFING')).upper()} - {today_str()}",
            "=" * 72,
            CREATOR_FOOTNOTE,
            "EXECUTIVE SUMMARY",
